...
<script>
    export default {
      ...
      methods:{
        loadData() {
          this.$http.get('http://172.27.49.77:9999/api/Crawler/jobs')
            .then(res => {
              this.jobs = res;
            });
        },
        refreshJob(job){
          this.$http.get('http://172.27.49.77:9999/api/Crawler/jobs/'+job.name)
            .then(res => {
        ...
        runJob(job){
          if(job.running){
            ...
            this.$http.get('http://172.27.49.77:9999/api/Crawler/run/'+job.name)
              .then(res => {
